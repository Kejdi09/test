...
const CartContent = () => {
  ...
  // ✅ Empty cart handling immediately
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-20 text-center">
          ...
          <p className="font-body text-muted-foreground mb-8">{t.cartEmptyMessage}</p>
          <Link to="/shop">
            <Button className="btn-primary font-body uppercase tracking-wider">{t.continueShoppingBtn}</Button>
          </Link>
        </div>
      </Layout>
    );
  ...
  // ✅ Order message safe even if items is empty
  const orderMessage = useMemo(() => {
    ...
    `\n${t.subtotalLabel}: $${totalPrice.toFixed(2)}\n${t.shippingLabel}: ${
      shipping === 0 ? t.freeShippingText : `$${shipping.toFixed(2)}`
    }\n${t.totalLabel}: $${total.toFixed(2)}`;
  }, [items, totalPrice, shipping, total, t]);
            ...
            <div className="space-y-4 font-body text-sm">
              ...
              {shipping > 0 && (
                <p className="text-xs text-accent">
                  {t.addMoreForFreeShippingText.replace('${amount}', (150 - totalPrice).toFixed(2))}
                </p>
              )}
